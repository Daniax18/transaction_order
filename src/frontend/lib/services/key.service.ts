...
import type { UserKeys, GenerateKeyDto } from "@/lib/types/key";
...
export const keyService = {
   ...
   */
  async listKeys(userId?: string): Promise<UserKeys[]> {
   ...
   * @param id - ID de la clé à révoquer
   */
  async revokeKey(id: number): Promise<void> {
    await api.post("/keys/revoke", id);
  },
